...
import {
  deleteQuotationRemark,
  fetchQuotationRemarks,
  insertQuotationRemark,
  setDefaultQuotationRemark,
  updateQuotationRemark,
  type QuotationRemarkRow,
} from './quotationRemarksApi'

export function ManageQuotationRemarksDialog({
  open,
  onOpenChange,
  onChanged,
  onDefaultSet,
  ...
  onChanged?: (remarks: QuotationRemarkRow[], defaultContent: string) => void
  onDefaultSet?: (content: string) => void
}) {
  const [remarks, setRemarks] = useState<QuotationRemarkRow[]>([])
  ...
  const [viewingRemark, setViewingRemark] = useState<QuotationRemarkRow | null>(null)
  ...
  const reload = async () => {
    ...
    setError(null)
    try {
      const rows = await fetchQuotationRemarks()
      setRemarks(rows)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateQuotationRemark(editingId, label, content)
        if (setAsDefault) {
          await setDefaultQuotationRemark(editingId)
          onDefaultSet?.(content)
        }
      } else {
        const makeDefault = setAsDefault || remarks.length === 0
        await insertQuotationRemark(label, content, makeDefault)
  ...
  const handleSetDefault = async (id: string) => {
    ...
    setError(null)
    try {
      await setDefaultQuotationRemark(id)
      const content = remarks.find((n) => n.id === id)?.content
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null)
    try {
      const wasDefault = remarks.find((n) => n.id === id)?.isDefault
      await deleteQuotationRemark(id)
      const remaining = remarks.filter((n) => n.id !== id)
      if (wasDefault && remaining[0]) {
        await setDefaultQuotationRemark(remaining[0].id)
      }
        ...
        >
          ...
          <div className="space-y-4 bg-gradient-to-b from-stone-100/80 to-white px-4 py-4">
            <div className="space-y-3 rounded-none border border-stone-500 bg-stone-50/70 p-3">
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="quotation-remark-text"
                  className="text-[11px] font-semibold uppercase tracking-wide text-stone-600"
                >
                  Remark Text
                </Label>
                <Textarea
                  id="quotation-remark-text"
                  value={draftText}
                        ...
                        <td className="w-36 px-3 py-2 align-middle">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                window.requestAnimationFrame(() => {
                                  document.getElementById('quotation-remark-label')?.focus()
                                })
                              }}
                            >
                            ...
                            <button
                              ...
                              title="Delete"
                              disabled={saving || remarks.length <= 1}
