...
export function MaintenanceScheduleTable({
  ...
  onViewLastMaintenance?: (row: MaintenanceScheduleRow) => void
}) {
  ...
  return (
    <div className={cn(limsPanelClass, 'bg-[#f7f3eb]')}>
      ...
      {loading ? (
        <p className="px-5 py-8 text-center text-sm text-[#78716c]">Loading maintenance schedule…</p>
      ) : rows.length === 0 ? (
          ...
          <Table className={GRID_TABLE}>
            <TableHeader>
              <TableRow className="border-stone-700 bg-stone-800 hover:bg-stone-800">
                ...
                <TableHead className={thBase}>Last Maintenance</TableHead>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="align-middle">
                      <p className={primaryLineClass}>{r.equipmentName || '—'}</p>
                    </TableCell>
                    <TableCell className="align-middle text-center">
                    ...
                    <TableCell className="align-middle text-center">
                      {r.lastMaintenanceDate ? (
                        <button
                          ...
                          className={cn(
                            primaryLineClass,
                            'text-amber-800 underline-offset-2 hover:text-amber-950 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/40',
                          )}
                          onClick={() => onViewLastMaintenance?.(r)}
                          aria-label={`View last maintenance details for ${r.equipmentName || r.assetCode}`}
                        >
                          {formatDateDisplay(r.lastMaintenanceDate)}
                        </button>
                      ) : (
                        <span className={primaryLineClass}>—</span>
                      )}
                    </TableCell>
                    <TableCell className="align-middle text-center">
                      <span className={primaryLineClass}>
                        {formatDateDisplay(r.nextMaintenanceDate)}
                      </span>
                    </TableCell>
                    <TableCell className="align-middle text-center">
                      <span
