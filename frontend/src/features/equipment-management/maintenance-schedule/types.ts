...
export function mapEquipmentToScheduleRow(input: {
  source: MaintenanceSource
  id: string
  ...
  const frequency = String(input.maintenance_schedule_frequency ?? '').trim()
  const last = toDateOnly(input.last_maintenance_date)
  const next = resolveNextMaintenanceDate(last, frequency, toDateOnly(input.next_maintenance_date))
...
export function dueLabel(row: MaintenanceScheduleRow): string {
