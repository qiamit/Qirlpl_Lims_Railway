...
import type { DueBucket, MaintenanceSource } from './types'

export function MaintenanceScheduleHeaderBar({
  search,
  onSearchChange,
  pageSize,
  onPageSizeChange,
  ...
  sourceFilter: 'all' | MaintenanceSource
  onSourceFilterChange: (value: 'all' | MaintenanceSource) => void
            ...
            <div className="order-3 flex w-full min-w-0 flex-wrap items-center gap-2 sm:order-none sm:ml-2 sm:w-auto">
              <div className="relative min-w-0 flex-1 sm:w-[16rem] sm:flex-none">
                ...
                <Input
                  ...
                  aria-label="Search maintenance schedule"
                />
              </div>

              <Select
                value={sourceFilter}
                onValueChange={(v) => onSourceFilterChange(v as 'all' | MaintenanceSource)}
              >
                <SelectTrigger
