...
export function AiSettingsHeaderBar({
  ...
  return (
    <div className="space-y-4 rounded-xl border border-border bg-card px-5 py-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="shrink-0 text-lg font-semibold tracking-tight text-foreground">AI Settings</h1>
        ...
        <div className="flex w-full flex-col gap-3 sm:max-w-xl sm:flex-1 sm:flex-row sm:items-center sm:justify-end">
          ...
          {activeTab === 'models' ? (
            <Button type="button" className="gap-2 shrink-0" size="sm" onClick={onPrimaryAction} aria-label="Add Model">
              <Plus size={14} />
              Add Model
            </Button>
          ) : null}
          ...
          {activeTab === 'skills' ? (
            <Button type="button" className="gap-2 shrink-0" size="sm" onClick={onPrimaryAction} aria-label="Add Skill">
              <Plus size={14} />
              Add Skill
            </Button>
          ) : null}
