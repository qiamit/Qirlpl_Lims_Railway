 ...
 *  1     : TC (fixed; counts as one position, two characters)
...
const NABL_ULR_PATTERN = new RegExp(
  `^TC\\d{${NABL_ULR_CERT_DIGITS}}\\d{${NABL_ULR_YEAR_DIGITS}}[0-9A-F][0-9A-F]{${NABL_ULR_SERIAL_DIGITS}}${NABL_ULR_LAST_CHAR}$`,
)
...
/**
 * Lab prefix is typically TC + cert (5) + year (2), optionally + location (1).
 * Returns the 10-char header used before the 8-digit serial.
 */
export function normalizeNablUlrHeaderPrefix(prefix: string): string {
  let p = normalizeUlrAlphanumeric(prefix)
  if (!p.startsWith(NABL_ULR_FIXED_PREFIX)) {
    p = `${NABL_ULR_FIXED_PREFIX}${p}`
  }

  const minHeaderLen = NABL_ULR_FIXED_PREFIX.length + NABL_ULR_CERT_DIGITS + NABL_ULR_YEAR_DIGITS
 ...
 * Header = TC + cert (5) + year (2) + location (1); serial starts at 1 each calendar year.
 */
export function formatNablUlrNumber(prefix: string, sequence: number): string {
  const header = normalizeNablUlrHeaderPrefix(prefix)
