...
function statusBadgeLabel(row: TestAllocationRow): string {
  if (row.sampleStage === 'completed') return 'Issued'
  if (row.sampleStage === 'report_preparation') return 'Report Preparation'
  if (sectionWasApprovedForReview(row)) return 'Reviewed'
  return 'Reviewed'
}

function statusSortRank(row: TestAllocationRow): number {
  const badge = statusBadgeLabel(row)
  if (badge === 'Reviewed') return 0
  if (badge === 'Report Preparation') return 1
  if (badge === 'Issued') return 2
  return 3
}

function compareReviewedRows(a: TestAllocationRow, b: TestAllocationRow): number {
...
export function ResultsReviewedSrfsDialog({
  open,
  onOpenChange,
  rows,
}: {
  ...
  const filteredRows = useMemo(() => {
    ...
    return list.sort(compareReviewedRows)
  }, [rows, search])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-[#f7f3eb] p-4 sm:p-5">
          {rows.length === 0 ? (
            <p className="text-sm text-stone-600">No reviewed results yet. Approve a section to move it here.</p>
          ...
          <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap items-center gap-3 text-xs text-stone-300">
              ...
              {search.trim() ? (
                ...
                <span>{rows.length} section(s)</span>
              )}
            </div>
