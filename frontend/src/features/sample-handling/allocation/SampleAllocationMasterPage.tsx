...
export default function SampleAllocationMasterPage() {
  ...
  const [sortKey, setSortKey] = useState<SampleAllocationSortKey>('srfNumber')
  ...
  const filteredAllocated = useMemo(
    () => rows.filter((r) => r.allocationIds.length > 0),
    [rows],
  )
  ...
  const toggleAll = (checked: boolean, sampleIds?: string[]) =>
    setSelectedIds((prev) => {
      const n = new Set(prev)
      const targets = sampleIds ?? sortedAllocated.map((r) => r.sampleId)
      targets.forEach((id) => (checked ? n.add(id) : n.delete(id)))
      return n
    })
  ...
  return (
    <div className={limsPageShellClass}>
      <SampleAllocationHeaderBar
        ...
        allocatedRows={sortedAllocated}
        selectedIds={selectedIds}
        onToggle={toggleRow}
        onToggleAll={toggleAll}
        sampleAllocationIdsWithTestAllocation={sampleAllocationIdsWithTestAllocation}
        onEditAllocated={handleEdit}
        onReferbackAllocated={handleReferbackToReceiving}
        onSendAllocatedToTestAllocation={handleSendToTestAllocation}
      />
