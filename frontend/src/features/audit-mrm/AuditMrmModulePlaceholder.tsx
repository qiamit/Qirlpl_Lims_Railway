...
/** Shared placeholder shell for Audit & MRM Management modules. */
export function AuditMrmModulePlaceholder({
  title,
  description,
}: {
  title: string
    ...
    <div className="mx-auto flex w-full max-w-[1600px] flex-1 items-center justify-center p-6">
      <div className="flex max-w-md flex-col items-center gap-4 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-teal-500/10 ring-1 ring-teal-500/20">
          <ClipboardCheck size={28} className="text-teal-700" aria-hidden />
        </div>
        <div className="space-y-1.5">
          <h1 className="text-xl font-semibold text-foreground">{title}</h1>
          <p className="text-sm text-muted-foreground">Audit & MRM Management</p>
        </div>
