...
/** Shared placeholder shell for Complaints Management modules (ISO 17025 §7.9). */
export function ComplaintsModulePlaceholder({
  title,
  description,
}: {
  title: string
    ...
    <div className="mx-auto flex w-full max-w-[1600px] flex-1 items-center justify-center p-6">
      <div className="flex max-w-md flex-col items-center gap-4 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-500/10 ring-1 ring-amber-500/20">
          <MessageSquareWarning size={28} className="text-amber-800" aria-hidden />
        </div>
        <div className="space-y-1.5">
          <h1 className="text-xl font-semibold text-foreground">{title}</h1>
          <p className="text-sm text-muted-foreground">Complaints Management · ISO 17025 §7.9</p>
        </div>
