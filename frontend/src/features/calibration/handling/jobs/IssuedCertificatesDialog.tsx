...
function formatError(err: unknown) {
  if (!err || typeof err !== 'object') return 'Unable to load issued certificates'
  return (err as { message?: string }).message ?? 'Unable to load issued certificates'
}

export function IssuedCertificatesDialog({
  open,
  onOpenChange,
}: {
  open: boolean
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void fetchCalibrationJobsByStage('certificates')
      .then((data) => {
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-gradient-to-b from-stone-100/90 to-stone-50 p-[1%]">
          <CalibrationJobStageTable
            ...
            hideAction
            emptyMessage="No issued certificates yet. Issue a certificate from Certificate Preparation to see it here."
          />
        </div>
      </DialogContent>
    </Dialog>
