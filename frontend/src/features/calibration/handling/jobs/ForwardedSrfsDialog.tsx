...
import {
  ...
  referbackCalibrationJobsToJobAllocation,
...
const FORWARDED_STAGES: CalibrationJobStage[] = [
  'calibration_conduct',
  'review_data',
...
function formatError(err: unknown) {
  if (!err || typeof err !== 'object') return 'Unable to load forwarded SRFs'
  return (err as { message?: string }).message ?? 'Unable to load forwarded SRFs'
}

export function ForwardedSrfsDialog({
  open,
  onOpenChange,
  onChanged,
}: {
  ...
  useEffect(() => {
    ...
    void fetchCalibrationJobsByStage(FORWARDED_STAGES)
      .then((data) => {
  ...
  const groups = useMemo(() => groupCalibrationJobsBySrf(filteredRows), [filteredRows])
      ...
      >
        <div className="relative shrink-0 bg-gradient-to-br from-stone-800 via-stone-900 to-stone-950 px-3 py-2.5 text-white sm:px-5 sm:py-3">
          ...
          <div className="relative flex flex-wrap items-center gap-2 pr-10 sm:flex-nowrap sm:gap-3">
            ...
            <div className="order-3 w-full min-w-0 sm:order-none sm:mx-1 sm:w-[40%] sm:max-w-[19.5rem] sm:flex-none">
              <Input
                type="search"
                placeholder="Search forwarded SRFs"
                aria-label="Search forwarded SRFs"
                value={search}
            ...
            <p className="ml-auto hidden text-xs text-stone-300 sm:block">
              {groups.length} {groups.length === 1 ? 'SRF' : 'SRFs'}
            </p>
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-gradient-to-b from-stone-100/90 to-stone-50 p-[1%]">
          <CalibrationJobStageTable
            ...
            onToggleAll={(checked) => {
              setSelectedSrfIds(
                checked ? new Set(pagedGroups.map((g) => g.serviceRequestId)) : new Set(),
              )
            }}
            onForward={() => undefined}
            onReferback={(group) => {
              void (async () => {
                ...
                try {
                  await referbackCalibrationJobsToJobAllocation(ids)
                  setRows((prev) => prev.filter((r) => !ids.includes(r.id)))
                  setSelectedSrfIds((prev) => {
                    const next = new Set(prev)
                    next.delete(group.serviceRequestId)
                    return next
                  })
                  onChanged?.()
                } catch (err) {
            ...
            hideAction
            emptyMessage="No forwarded SRFs yet. Forward an SRF from Job Allocation to see it here."
          />
        </div>
      </DialogContent>
    </Dialog>
