...
import type { CalibrationJobLocation, CalibrationJobRow, CalibrationJobStage } from '../types'
...
function formatError(err: unknown) {
  if (!err || typeof err !== 'object') return 'Unable to load forwarded equipment'
  return (err as { message?: string }).message ?? 'Unable to load forwarded equipment'
}

export function ForwardedEquipmentDialog({
  open,
  onOpenChange,
  locationFilter,
}: {
  ...
  useEffect(() => {
    ...
    void fetchCalibrationJobsByStage(FORWARDED_STAGES, {
      calibrationLocation: locationFilter ?? null,
    })
      .then((data) => {
      ...
      >
        <div className="relative shrink-0 bg-gradient-to-br from-stone-800 via-stone-900 to-stone-950 px-3 py-2.5 text-white sm:px-5 sm:py-3">
          ...
          <div className="relative flex flex-wrap items-center gap-2 pr-10 sm:flex-nowrap sm:gap-3">
            ...
            <div className="order-3 w-full min-w-0 sm:order-none sm:mx-1 sm:w-[40%] sm:max-w-[19.5rem] sm:flex-none">
              <Input
                type="search"
                placeholder="Search forwarded equipment"
                aria-label="Search forwarded equipment"
                value={search}
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-gradient-to-b from-stone-100/90 to-stone-50 p-[1%]">
          <CalibrationConductTable
            ...
            viewOnly
            showOutsideChecklists={locationFilter === 'On Site'}
            emptyMessage="No forwarded equipment yet. Forward equipment from Calibration Conduct to see it here."
          />
        </div>
      </DialogContent>
    </Dialog>
